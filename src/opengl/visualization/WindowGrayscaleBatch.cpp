...
void WindowGrayscaleBatch::displayHandler(void){
	...
	if (batch_data.size() > 0){
		...
		size_t batch_height = ceil((float)batch_data.size()/batch_width);
...
void WindowGrayscaleBatch::setBatchDataSynchronized(std::vector <std::shared_ptr<mic::types::MatrixXf> > & batch_data_) {
	// Enter critical section.
	APP_DATA_SYNCHRONIZATION_SCOPED_LOCK();
...
void WindowGrayscaleBatch::setBatchDataUnsynchronized(std::vector <std::shared_ptr<mic::types::MatrixXf> > & batch_data_) {
